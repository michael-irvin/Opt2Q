...
"""
Case 11: TRAIL dependent (only) model. Calibrates apoptosis model to data where TRAIL, DISC formation and Caspase-8
activity are independent variables:
    (IV) TRAIL concentration, Bid expression modulation via knockdown, Caspase-8 Activity via zVAD
...
# or where BidKD is applied
data = data_file[data_file.Condition.str.match(r'^Control$') | data_file.Condition.str.match(r'^siCtrl$') |
                 data_file.Condition.str.match(r'^zVAD') | data_file.Condition.str.match(r'siCHIP')].dropna(axis=1, how='all')
...
n_iterations = 10000
now = dt.datetime.now()
model_name = f'PyDream_case_11_{now.year}{now.month}{now.day}'
