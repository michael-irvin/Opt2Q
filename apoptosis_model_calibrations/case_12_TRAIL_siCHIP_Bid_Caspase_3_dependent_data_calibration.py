...
"""
Case 12: TRAIL dependent (only) model. Calibrates apoptosis model to data where TRAIL, DISC stability, Caspase-3 activity
...
# or where BidKD is applied, or Caspase 3 or siCHIP.
data = data_file[data_file.Condition.str.match(r'^Control$') | data_file.Condition.str.match(r'^siCtrl$') |
                 data_file.Condition.str.contains(r'Caspase 3') | data_file.Condition.str.match(r'siCHIP')]\
...
n_iterations = 4000
now = dt.datetime.now()
model_name = f'PyDream_case_12_{now.year}{now.month}{now.day}'
