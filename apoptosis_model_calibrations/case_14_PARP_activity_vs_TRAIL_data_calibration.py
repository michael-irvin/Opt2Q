...
"""
Case 14: Calibrating apoptosis model to experiments investigating the affect of
...
# We only want measured values columns that correspond to cPARP and PARP observables
mv_columns = set(k for k, v in CompileData.model_observables.items() if set(v)-{'cPARP_obs', 'PARP_obs'} == set())
data_columns = set()
...
now = dt.datetime.now()
model_name = f'PyDream_case_14_{now.year}{now.month}{now.day}'
