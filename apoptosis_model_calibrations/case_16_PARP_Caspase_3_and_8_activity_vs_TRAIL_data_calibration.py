...
"""
Case 16: Calibrating apoptosis model to experiments investigating the affect of
    (IV) TRAIL concentration on
    (DV) PARP, and Caspase-3 and -8 activity
...
mv_columns = set(k for k, v in CompileData.model_observables.items() if set(v) -
                 {'cPARP_obs', 'PARP_obs', 'C3_active_obs', 'C3_inactive_obs',
                  'C8_active_obs', 'C8_inactive_obs'} == set())
data_columns = set()
...
now = dt.datetime.now()
model_name = f'PyDream_case_16_{now.year}{now.month}{now.day}'
