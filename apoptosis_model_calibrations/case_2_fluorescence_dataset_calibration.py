...
"""
Case 2: "quantitative only model", Calibrates apoptosis model to fluorescence measurements in the dataset:
    (IV) TRAIL concentration, Caspase-3 Inhibition, Bid knockdown (BidKD)
    (DV) DVED-ase and EITD-ase indicator integrates w.r.t. time caspase-3 and -8 activity and serves as a proxy
         for PARP and Bid cleavage.
"""
...
data = data_file[data_file['Measurement Model'].str.match(r'^Fluorescence$')].dropna(axis=1, how='all')
...
now = dt.datetime.now()
model_name = f'PyDream_case_2_{now.year}{now.month}{now.day}'
