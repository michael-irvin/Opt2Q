...
"""
Case 4: "western blot model" Calibrating apoptosis model to all western blot measurements in the dataset
    (IV) TRAIL concentration, DISC Stabilization, Caspase-8 Inhibition, Bid knockdown (BidKD)
...
# We only want rows in which the 'Measurement Model' column is 'FractionalKilling' or 'WesternBlot'.
data = data_file[data_file['Measurement Model'].str.match(r'^WesternBlot')].dropna(axis=1, how='all')
...
n_iterations = 4000
now = dt.datetime.now()
model_name = f'PyDream_case_4_{now.year}{now.month}{now.day}'
