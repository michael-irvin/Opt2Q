...
"""
Case 5: "fractional killing model" Calibrating apoptosis model to all fractional killing data in the dataset
...
# We only want rows in which the 'Measurement Model' column is 'FractionalKilling'.
data = data_file[data_file['Measurement Model'].str.match(r'^FractionalKilling$')].dropna(axis=1, how='all')
...
now = dt.datetime.now()
model_name = f'PyDream_case_5_{now.year}{now.month}{now.day}'
