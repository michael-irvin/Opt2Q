...
"""
Case 7: TRAIL and siCHIP model. Calibrates apoptosis model to data where TRAIL and siCHIP are independent variables:
    (IV) TRAIL concentration, siCHIP mediated DISC stabilization
...
# We want the rows in which the 'Condition' column is a control (i.e. TRAIL is the only independent variable) and/or
# where siCHIP is applied
data = data_file[data_file.Condition.str.match(r'^Control$') | data_file.Condition.str.match(r'^siCtrl$') |
                 data_file.Condition.str.match(r'^siCHIP No. 2$')].dropna(axis=1, how='all')
...
n_iterations = 10000
now = dt.datetime.now()
model_name = f'PyDream_case_7_{now.year}{now.month}{now.day}'
