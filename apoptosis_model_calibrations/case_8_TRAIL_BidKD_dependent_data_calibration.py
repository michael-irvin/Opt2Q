...
"""
Case 8: TRAIL and Bid model. Calibrates apoptosis model to data where TRAIL and BID are independent variables:
    (IV) TRAIL concentration, Bid expression modulation via knockdown
...
data = data_file[data_file.Condition.str.match(r'^Control$') | data_file.Condition.str.match(r'^siCtrl$') |
                 data_file.Condition.str.match(r'^BidKD$')].dropna(axis=1, how='all')
...
now = dt.datetime.now()
model_name = f'PyDream_case_8_{now.year}{now.month}{now.day}'
