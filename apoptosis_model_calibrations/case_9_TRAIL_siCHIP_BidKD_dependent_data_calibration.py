...
"""
Case 9: TRAIL, si-CHIP and Bid model. Calibrates apoptosis model to data where TRAIL, Bid and siCHIP are independent
variables:
    (IV) TRAIL concentration, siCHIP mediated DISC stabilization, and Bid expression via knockout
...
# We want the rows in which the 'Condition' column is a control (i.e. TRAIL is the only independent variable)
data = data_file[data_file.Condition.str.match(r'^Control$') | data_file.Condition.str.match(r'^siCtrl$') |
                 data_file.Condition.str.match(r'^siCHIP No. 2$') | data_file.Condition.str.match(r'^BidKD$')]\
...
neg_log_likelihood = generate_likelihood_fn(compiled_dataset, n_sims=20, n_timepoints=10)
...
n_iterations = 10000
now = dt.datetime.now()
model_name = f'PyDream_case_9_{now.year}{now.month}{now.day}'
