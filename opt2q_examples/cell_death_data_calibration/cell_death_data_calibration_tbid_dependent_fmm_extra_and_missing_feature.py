...
# Calibrating the apoptosis model to pcd data using fixed measurement model
#   that includes irrelevant feature and excludes relevant feature
...
true_params = np.load(os.path.join(parent_dir, 'true_params.npy'))
...
a = 4
tbid_classifier.set_params(**{'coefficients__apoptosis__coef_': np.array([[1.0, 0.25, 0.0]])*a,  # include 'unrelated'
...
now = dt.datetime.now()
model_name = f'apoptosis_model_tbid_cell_death_data_calibration_fmm_missing_and_extra_feature_{now.year}{now.month}{now.day}'


# ------- Likelihood Function ------#
@objective_function(target=synth_data, sim=sim, pre_processing=pre_processing,
                    tbid_cell_death_classifier=tbid_classifier, return_results=False, evals=0)
def likelihood(x):
    ...
    cv_term = abs(x[len(param_names)])**-0.5
    ...
    try:
        ...
        prediction = likelihood.tbid_cell_death_classifier.transform(
