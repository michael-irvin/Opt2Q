...
# Calibrating the apoptosis model to pcd data using fixed measurement model that includes irrelevant feature
import os
import numpy as np
import pandas as pd
...
a = 4
tbid_classifier.set_params(**{'coefficients__apoptosis__coef_': np.array([[1.0, 0.25, -1.0]])*a,  # include 'unrelated'
...
now = dt.datetime.now()
model_name = f'apoptosis_model_tbid_cell_death_data_calibration_fmm_extra_feature_{now.year}{now.month}{now.day}'
