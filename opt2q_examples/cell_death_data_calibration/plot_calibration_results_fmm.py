...
calibration_date = '20201018'  # calibration file name contains date string
calibration_tag = 'fmm_20201018'
...
for i, param in enumerate(model_param_names):
    ...
    ax_trace_list[i].set_ylabel(model_param_names[i], rotation=0, ha='right')
    ax_hist_list[i].axes.get_yaxis().set_visible(False)

i = len(model_param_names)
ax_trace_list.append(fig.add_subplot(gs[i, :3]))
ax_hist_list.append(fig.add_subplot(gs[i, 3]))
...
ax_trace_list[i].set_yticks([])
ax_trace_list[i].set_ylabel(population_param_name[0], rotation=0, ha='right')
ax_hist_list[i].axes.get_yaxis().set_visible(False)
...
# Normed tBID 90% Credible Interval Calculation
...
plt.title("Posterior KDE of Parameter 'kc0'. (Fixed Measurement Model)")
plt.show()
...
plt.title("Posterior KDE of Parameter 'kc0' coefficient of variation. (Fixed Measurement Model)")
plt.show()
...
plt.title("Normalized Extrinsic Noise Functions for Parameter 'kc0' \n "
          "using Maximum Posterior Sample (Fixed Measurement Model)")
plt.show()
...
plt.title('KDE plots of Cell Death Predictor, \n Time at maximal Bid truncation rate '
          '\n using Random Sample of Posterior (Fixed Measurement Model)')
plt.xlabel('Time at max Bid truncation rate (standardized)')
plt.ylabel('Density')
legend_elements = [Line2D([0], [0], color='k', alpha=0.5, label='10ng/mL TRAIL'),
                   Line2D([0], [0], color=cm.colors[1], alpha=0.5, label='50ng/mL TRAIL')]
...
plot.kde_of_features(ax, features_priors_populations[features_priors_populations['TRAIL_conc'] == '50ng/mL'], 'time',
                     color=cm.colors[1], alpha=0.5)
plt.title('KDE plots of Cell Death Predictor, \n Time at Maximal Bid truncation rate '
          '\n using Random Sample of Prior (Fixed Measurement Model)')
plt.xlabel('Time at max Bid truncation rate (standardized)')
plt.ylabel('Density')
legend_elements = [Line2D([0], [0], color='k', alpha=0.5, label='10ng/mL TRAIL'),
                   Line2D([0], [0], color=cm.colors[1], alpha=0.5, label='50ng/mL TRAIL')]
...
plt.title('Measurement Model Predicted in the Calibration, (Fixed Measurement Model)')
...
legend_elements = [Line2D([0], [0], color=cm.colors[7], alpha=0.5, label='10ng/mL TRAIL'),
                   ...
                   Line2D([0], [0], color='b', alpha=0.5, label='assumed 50% probability line')]
plt.legend(handles=legend_elements)
plt.xlim(-3.8, 3.8)
plt.show()
