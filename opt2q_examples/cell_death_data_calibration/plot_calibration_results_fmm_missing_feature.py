...
calibration_tag = 'fmm_missing_feature_2020910'
...
plt.suptitle('Convergence and Parameter Values for Cell Death Data \n '
             'Calibration (Fixed Measurement Model w/ Missing Feature)')
...
plt.suptitle('Parameter Value Traces and Histograms \n '
             'Cell Death Data Calibration (Fixed Measurement Model w/ Missing Feature)', y=0.95, fontsize=16)
plt.show()

fig, ax = plt.subplots()
ax.set_title('tBID Simulation using Random Sample from Posterior of Model Trained \n to Cell Death Data '
             '(Fixed Measurement Model w/ Missing Feature)')
...
plt.title("Posterior KDE of Parameter 'kc0'. (Fixed Measurement Model w/ Missing Feature)")
plt.show()
...
plt.title("Normalized Extrinsic Noise Functions for Parameter 'kc0' \n "
          "using Maximum Posterior Sample (Fixed Measurement Model w/ Missing Feature)")
plt.show()
...
plt.title('KDE plots of Cell Death Predictor, \n Time at maximal Bid truncation rate '
          '\n using Random Sample of Posterior (Fixed Measurement Model w/ Missing Feature)')
plt.xlabel('Time at max Bid truncation rate (standardized)')
plt.ylabel('Density')
legend_elements = [Line2D([0], [0], color='k', alpha=0.5, label='10ng/mL TRAIL'),
                   Line2D([0], [0], color=cm.colors[1], alpha=0.5, label='50ng/mL TRAIL')]
...
plot.kde_of_features(ax, features_priors_populations[features_priors_populations['TRAIL_conc'] == '50ng/mL'], 'time',
                     color=cm.colors[1], alpha=0.5)
plt.title('KDE plots of Cell Death Predictor, \n Time at Maximal Bid truncation rate '
          '\n using Random Sample of Prior (Fixed Measurement Model w/ Missing Feature)')
plt.xlabel('Time at max Bid truncation rate (standardized)')
plt.ylabel('Density')
legend_elements = [Line2D([0], [0], color='k', alpha=0.5, label='10ng/mL TRAIL'),
                   Line2D([0], [0], color=cm.colors[1], alpha=0.5, label='50ng/mL TRAIL')]
...
plot.measurement_model_sample(ax, np.array([[4.00, -0.25, 0.00, 0.25, 0.00]]), 'tBID_obs', 'time', 0.5,
...
plot.measurement_model_sample(ax, np.array([[4.00, -0.25, 0.00, 0.25, 0.00]]), 'tBID_obs', 'Unrelated_Signal', 0.5,
                              np.linspace(-4, 4, 100), color='b', alpha=0.5)
plot.population_kde_of_features(ax, features_best_populations[features_best_populations.TRAIL_conc == '10ng/mL'],
