...
# Calibrating the apoptosis model to immunoblot data using fixed measurement model
import os
import numpy as np
import pandas as pd
...
def likelihood_fn(x):
    ...
    if hasattr(likelihood_fn.simulator.sim, 'gpu'):
        ...
        likelihood_fn.simulator.sim.gpu = [2]
...
now = dt.datetime.now()
model_name = f'immunoblot_data_calibration_fmm_{now.year}{now.month}{now.day}'
