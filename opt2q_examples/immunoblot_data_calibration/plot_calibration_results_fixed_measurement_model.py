...
elif 'fmm' == calibration_tag:
    best_measurement_model_parameters = measurement_model_param_true
    random_post_measurement_model_parameters = measurement_param_names
else:
    best_measurement_model_parameters = measurement_model_param_true
    random_post_measurement_model_parameters = np.array([[50, 0.03, 0.37, 0.42, 0.15, 50, 0.03, 0.17, 0.77]])
...
plt.suptitle('Convergence and Parameter Values for Ordinal Data \n '
             'Calibration (ad hoc Measurement Model)')
...
tBID_results = lc_results.filter(regex='tBID_blot')
ax2.set_title('ad hoc Measurement Model')
...
ax1.set_title('Normalized tBID 90% Credible Interval using Random Sample from Posterior '
              '\n of Model Trained to Ordinal Data, Using ad hoc Measurement Model')
