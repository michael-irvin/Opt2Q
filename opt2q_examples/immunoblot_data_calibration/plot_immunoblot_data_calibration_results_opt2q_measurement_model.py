...
from measurement_model_demo.calibrating_apoptosis_model_and_immunoblot_measurement_model import \
...
plot_title = 'Immunoblot Calibration (apoptosis dynamics and measurement model)'
...
gr_file = sorted([os.path.join(script_dir, calibration_folder, f) for f in
                  os.listdir(os.path.join(script_dir, calibration_folder))
                  if '2020118' in f and file_order[-1] in f and '.txt' in f])[0]
...
all_true_params = list(true_params) + [50, 0.03, 0.37, 0.42, 0.15, 50, 0.03, 0.17, 0.77]
all_parameters = param_names + \
                 ['coefficients__tBID_blot__coef_', 'coefficients__tBID_blot__theta_1 ',
...
params_of_interest = range(len(all_parameters))

# =====================================
# convergence
# plt.figure(figsize=(10, 3))
plt.bar(x=all_parameters[:17], height=gr[:17])
plt.axhline(y=1.2, linestyle='--', color='k', alpha=0.5)
plt.title('Gelman Rubin convergence metric for model parameters')
plt.show()

plt.bar(x=all_parameters[17:-10], height=gr[17:-10])
plt.axhline(y=1.2, linestyle='--', color='k', alpha=0.5)
plt.title('Gelman Rubin convergence metric for model parameters')
plt.show()

plt.bar(x=all_parameters[-10:], height=gr[-10:])
plt.axhline(y=1.2, linestyle='--', color='k', alpha=0.5)
plt.xticks(all_parameters[-10:], rotation='vertical')
plt.title('Gelman Rubin convergence metric for model parameters')
plt.show()
...
if __name__ == '__main__':
    ...
    for j in params_of_interest:
        s = pd.DataFrame({f'trace {n}': parameter_samples[n][burn_in_len:, j][::thin] for n in range(len(parameter_samples))})
        ax = s.plot.hist(alpha=0.5, bins=20)
        s.plot(kind='kde', ax=ax, secondary_y=True)

        # plt.hist(param_array[burn_in_len:, j], normed=True)
        plt.title(all_parameters[j])
        plt.show()
    ...
    for j, k in combinations(params_of_interest, 2):
        ...
        if any(x > 0.9 for x in param_cor):
            ...
            plt.xlabel(all_parameters[j])
            plt.ylabel(all_parameters[k])
            plt.show()
...
if __name__ == '__main__':
    ...
    for idx in params_of_interest[:len(param_names)]:
    ...
    for idx in params_of_interest[len(param_names):-1]:
        s = pd.DataFrame({f'{all_parameters[idx]}': params[:, idx][::thin]})
        ax1 = s.plot.hist(alpha=0.4, bins=20, color='k')
        s.plot(kind='kde', ax=ax1, color=cm.colors[1], secondary_y=True)

        x_range = np.linspace(1.1*s.min(), 1.1*s.max(), 1000)
        prior = sampled_params_0[idx-len(param_names)+1]

        ax2 = ax1.twinx()
        ax2.spines["right"].set_position(("axes", 1.2))
        ax2.plot(x_range, prior.dist.pdf(x_range), '--', color='k', label=f'{all_parameters[idx]}, prior')
        ax2.axvline(x=all_true_params[idx], color='k', alpha=0.5, label='true mean')
        ax2.set_ylabel('Density (prior)')
        ...
        plt.legend(handles=handles, bbox_to_anchor=(0.5, -0.6, 0.5, 0.5))
        plt.title(f'Calibrated {all_parameters[idx]} Value')
        plt.show()
...
lc_params = [[p for p in params[idx, num_params:]] for idx in ids]
...
if __name__ == '__main__':
    ...
    c_id = 0
    for col in sorted(list(tBID_results.columns)):
        ax3.plot(tBID_results[col].values, np.linspace(0, 1, 100), alpha=0.7, color=cm.colors[c_id],
                 label=col)
        c_id += 1
...
if __name__ == '__main__':
    ...
    for row_id in range(len(lc_params)):
        ...
        for col in sorted(list(cPARP_results.columns)):
            ax2.plot(cPARP_results[col].values, np.linspace(0, 1, 100), alpha=0.1, color=cm.colors[ci])
            ci += 1
    ...
    c_id = 0
    for col in sorted(list(cPARP_results.columns)):
        ax3.plot(cPARP_results[col].values, np.linspace(0, 1, 100), alpha=0.7, color=cm.colors[c_id], label=col)
        c_id += 1
